(function(){
  const storeRoutes={
    '7-11':'/7-eleven/','全家':'/familymart/','萊爾富':'/hilife/','全聯':'/pxmart/','萬家福':'/wanjiafu/','樂家康':'/lejiakang/'
  };

  function initStoreCatalogLinks(){
    document.querySelectorAll('.store-line .store-tag').forEach(tag=>{
      const label=tag.textContent.trim(),href=storeRoutes[label];
      if(!href||tag.tagName==='A')return;
      const link=document.createElement('a');link.className=tag.className+' store-tag-link';link.href=href;
      ...
      link.append(text,arrow);tag.replaceWith(link);
    });
    if(!document.querySelector('#store-catalog-link-style')){
      const style=document.createElement('style');style.id='store-catalog-link-style';
  ...
  function render(input,showAll=false){
    const box=input.parentElement.querySelector('[data-search-results]');const q=input.value.trim();
    if(!q){box.innerHTML='';state.delete(input);return}
    const hits=engine.search(index,q);state.set(input,{q,hits});
    ...
    if(!hits.length){box.innerHTML='<div class="search-row"><span><b>目前沒找到</b><small>可改用較短的品牌或商品名稱，或查看完整通路清單。</small></span></div>';return}
    ...
    box.innerHTML=`<div class="search-count">找到 ${hits.length} 筆</div>${rows}${more}${collapse}`;
  }
  ...
  if(initial){
    ...
    if(input){input.value=initial;render(input,false);setTimeout(()=>document.querySelector('#search')?.scrollIntoView({block:'start'}),50)}
  }
})();
